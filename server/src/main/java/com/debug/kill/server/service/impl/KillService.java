...
public class KillService implements IKillService {
    ...
    public Boolean killItem(Integer killId, Integer userId) throws Exception {
        ...
        //TODO:判断当前用户是否已经抢购了当前商品   //根据用户名和商品名在秒杀成功的数据库中查数据条数  <=0说明还没有秒杀过 （这里判断了status=0  说明秒杀成功不付款的人不能再次秒杀）
        if (itemKillSuccessMapper.countByKillUserId(killId,userId) <= 0){
            //TODO:判断当前代抢购的商品库存是否充足、以及是否出在可抢的时间段内 - canKill   cankill=1表示可以秒杀
    ...
    private void commonRecordKillSuccessInfo(ItemKill kill, Integer userId) throws Exception{
        ...
        entity.setCode(orderNo); //雪花算法    直接拿来用的
    ...
    public Boolean killItemV2(Integer killId, Integer userId) throws Exception {
        ...
        if (itemKillSuccessMapper.countByKillUserId(killId,userId) <= 0){
            //TODO:A.查询待秒杀商品详情
            ItemKill itemKill=itemKillMapper.selectByIdV2(killId);    //selectByIdV2已经限制了a.total>0 下面其实可有可无

            //TODO:判断是否可以被秒杀canKill=1?
            if (itemKill!=null && 1==itemKill.getCanKill() && itemKill.getTotal()>0){ //这里进行了mysql的优化  查cankill表示可以秒杀，同时保证可秒杀的数量>0
                                                                             // cankill表示了时间   total表示数量也可以
                //TODO:B.扣减库存-减一
                int res=itemKillMapper.updateKillItemV2(killId);   //这个v2 也是保证了total>0的时候才可以减   res难道是返回sql语句执行是否成功
                                                                                          // =1的话就是说明1行数据受到影响
                                                                                         //=0的话应该是说没有数据受到影响，应该是这样的

                //TODO:扣减是否成功?是-生成秒杀成功的订单，同时通知用户秒杀成功的消息
                if (res>0){
                    commonRecordKillSuccessInfo(itemKill,userId);   //这里就是简单的生成一个新消息嘛
    ...
    public Boolean killItemV3(Integer killId, Integer userId) throws Exception {
        ...
        if (itemKillSuccessMapper.countByKillUserId(killId,userId) <= 0){    //这一步包不包括在cache中都是可以的   包含的话包含的力度太大就会造成性能降低

            //TODO:借助Redis的原子操作实现分布式锁-对共享操作-资源进行控制
            ValueOperations valueOperations=stringRedisTemplate.opsForValue();
            //构造key和value
            //key  killid和userid      一个用户抢一个商品
            //value  直接用生成的订单编号就可以   value其实没有实际含义
            final String key=new StringBuffer().append(killId).append(userId).append("-RedisLock").toString();
            final String value=RandomUtil.generateOrderCode();
            Boolean cacheRes=valueOperations.setIfAbsent(key,value); //lua脚本提供“分布式锁服务”，就可以写在一起
            //TOOD:redis部署节点宕机了
            if (cacheRes){
                stringRedisTemplate.expire(key,30, TimeUnit.SECONDS);   //一会看一下这个函数
                ...
                try {    //查询判断更新记录
                    ...
                    if (value.equals(valueOperations.get(key).toString())){     //释放锁  通过value进行释放  保证释放的锁就是当时获取的锁
